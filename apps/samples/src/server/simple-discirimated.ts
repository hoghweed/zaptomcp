...
server.decorate("stytchClient", stytchClient);
...
app.post("/mcp", async (req, res) => {
  ...
  try {
    ...
    const token = req.headers["session-token"] as string | undefined;
    const sessionId = req.headers["mcp-session-id"] as string | undefined;
    let transport: StreamableHTTPServerTransport;

    const session = await server.stytchClient.magicLinks.authenticate({
      magic_links_token: token as string,
      session_duration_minutes: 60,
    });

    req.headers.discriminator = session.organization_id;
