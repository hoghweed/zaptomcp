...
import { JSONRPCMessageSchema, type JSONRPCMessage } from "../../types.js";
import { auth, UnauthorizedError, type AuthResult, type OAuthClientProvider } from "../auth.js";
...
export class SSEClientTransport implements Transport {
  ...
  private async _commonHeaders(): Promise<{ [index: string]: string}> {
  ...
  async send(message: JSONRPCMessage): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(this._endpoint, { ...init, signal: init.signal ?? null });
      if (!response.ok) {
