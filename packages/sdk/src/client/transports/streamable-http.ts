...
import { auth, type AuthResult, type OAuthClientProvider, UnauthorizedError } from "../auth.js";
...
export class StreamableHTTPClientTransport implements Transport {
  ...
  private _authProvider?: OAuthClientProvider | undefined;
  ...
  private async _commonHeaders(): Promise<Headers> {
    ...
    return new Headers(
      { ...Object.fromEntries(headers), ...this._requestInit?.headers }
    );
  }
  ...
  private _getNextReconnectionDelay(attempt: number): number {
    ...
    return Math.min(initialDelay * growFactor ** attempt, maxDelay);
  ...
  async send(message: JSONRPCMessage, options?: { relatedRequestId?: RequestId }): Promise<void> {
    try {
      ...
      if (relatedRequestId) {
        ...
        this._startOrAuthSse({ resumptionToken: undefined, replayMessageId: isJSONRPCRequest(message) ? message.id : undefined }).catch(err => this.onerror?.(err));
        return;
      }
      ...
      // Set signal to null if undefined to match RequestInit type
      init.signal = init.signal ?? undefined;

      const response = await fetch(this._url, init as RequestInit);
      ...
      if (hasRequests) {
        if (contentType?.includes("text/event-stream")) {
          ...
          this._handleSseStream(response.body, { onresumptiontoken: undefined });
