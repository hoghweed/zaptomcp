...
describe("StreamableHTTPClientTransport", () => {
  ...
  it("should send batch messages", async () => {
    ...
    await transport.send(messages[0] as JSONRPCMessage); // Send first message since send() expects single message
  ...
  it("should attempt initial GET connection and handle 405 gracefully", async () => {
    ...
    await expect(transport["_startOrAuthSse"]({})).resolves.not.toThrow(
      "Failed to open SSE stream: Method Not Allowed"
    );
  ...
  it("should handle successful initial GET connection for SSE", async () => {
    ...
    // biome-ignore lint/complexity/useLiteralKeys: <explanation>
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        await (transport as any)["_startOrAuthSse"]({});
  ...
  it("should handle multiple concurrent SSE streams", async () => {
    ...
    expect(
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
      messageSpy.mock.calls.some((call: any) => {
        const msg = call[0];
  ...
  it("should always send specified custom headers", async () => {
    ...
    (global.fetch as any).mockImplementation(
      async (_url: string, reqInit: RequestInit) => {
    ...
    expect((actualReqInit.headers as Headers).get("x-custom-header")).toBe(
      "CustomValue"
    );
