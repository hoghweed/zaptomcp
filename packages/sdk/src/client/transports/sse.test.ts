...
describe("SSEClientTransport", () => {
  ...
  describe("header handling", () => {
    ...
    it("passes custom headers to fetch requests", async () => {
      ...
      try {
        ...
        // Changed from jest.Mock to MockInstance from vitest
        const calledHeaders = (global.fetch as ReturnType<typeof vi.fn>).mock.calls?.[0]?.[1]
          .headers;
  ...
  describe("auth handling", () => {
    ...
    it("attempts auth flow on 401 during POST request", async () => {
      ...
      // Changed from mock.calls to Vitest's equivalent
      expect(mockAuthProvider.redirectToAuthorization).toHaveBeenCalledTimes(1);
    });
